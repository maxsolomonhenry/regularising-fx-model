...
def evaluate(cfg, model_type, nameModel):
    ...
    Xtest = test_h5('./data/Xval_test.h5')
    Ytest = test_h5('./data/Yval_test.h5')
    ...
    metrics['mae'] = str(round(np.mean(mae), 5))
    metrics['mfcc_cosine'] = str(round(np.mean(mfcc_cosine), 5))
    metrics['msed'] = str(round(np.mean(d['euclidean']), 5))
